...
public class LanguageVelocity {
    ...
    private static final CommentedConfigurationNode CONFIG = VelocityPlugin.getInstance().getConfig();
    ...
    public static void setupLanguage() {
        try {
            File file = new File(VelocityPlugin.getInstance().getDataDirectory().toFile(), "language.yml");
            ...
            VelocityPlugin.getInstance().getLogger().info("Configurando sistema de linguagem...");

            for (Field field : LanguageVelocity.class.getDeclaredFields()) {
                if (field.getName().contains("$") && !Modifier.isFinal(field.getModifiers())) {
                    String nativeName = field.getName().replace("$", ".").replace("_", "-");
                    String[] path = nativeName.split("\\.");

                    try {
                        CommentedConfigurationNode node = config.node((Object[]) path);
                        
                        if (!node.virtual()) {
                            Object value = node.get(String.class);
                        ...
                        VelocityPlugin.getInstance().getLogger().warn("Erro ao processar campo: " + nativeName, e);
                    }
                }
            }
            ...
            if (save) {
                loader.save(config);
                VelocityPlugin.getInstance().getLogger().info("Arquivo language.yml criado com sucesso!");
            } else {
                VelocityPlugin.getInstance().getLogger().info("Sistema de linguagem carregado com sucesso!");
            }

        } catch (Exception e) {
            VelocityPlugin.getInstance().getLogger().error("Erro fatal ao configurar sistema de linguagem", e);
        }
    }

    public static String getMessage(String key) {
        try {
            Field field = LanguageVelocity.class.getDeclaredField(key.replace(".", "$").replace("-", "_"));
            Object value = field.get(null);
            return value instanceof String ? (String) value : String.valueOf(value);
        } catch (Exception e) {
            VelocityPlugin.getInstance().getLogger().warn("Mensagem não encontrada: " + key);
