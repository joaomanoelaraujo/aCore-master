...
public class MenuCosmetic<T extends Cosmetic> extends PagedPlayerMenu {
  ...
  public MenuCosmetic(Profile profile, String name, Class<T> cosmeticClass) {
...
//    // Configuração dos itens do menu
    ...
    Optional<ColoredTag> selectedCosmetic2 = Cosmetic.listByType(ColoredTag.class)
            .stream()
            .map(f -> profile.getAbstractContainer("aCoreProfile", "cselected", SelectedContainer.class).getSelected(f.getType(), ColoredTag.class))
            .filter(Objects::nonNull)
            .findFirst();
    ...
    String coloredTagDesc = LanguageAPI.getConfig(profile).getString("menucosmetics.colored_tag")
            ...
            .replace("{max}", String.valueOf(max))
            .replace("{percentage}", String.valueOf(percentage))
            .replace("{selected}", sb2.toString().isEmpty() ? "\n &aNone" : sb2.toString());
          ...
          if (item != null && item.getType() != Material.AIR) {
            if (evt.getSlot() == this.previousPage) {
              ...
              if (cosmetic != null) {
                ...
                new MenuCosmetic<>(profile, this.name.replace("Profile - ", ""), this.cosmeticClass);
              }
            }
          }
        }
