...
implements INPC {
    ...
    public void despawn() {
        ...
        // Define os IDs das entidades a serem destruídas
        if (VersionUtil.isAbove(VersionUtil.VersionEnum.V1_17)) {
            // Usar setEntityIds diretamente para versões acima de 1.17
            destroyPacket.setEntityIds(new int[]{this.id});
        } else {
            destroyPacket.setEntityIds(new int[]{this.id});
        }

        // Envia o pacote para todos os jogadores visíveis
        for (Player player : Arrays.asList(this.visible)) {
            if (player == null) continue;
            destroyPacket.sendPacket(player);
        }

        // Limpa a lista de jogadores visíveis
        Arrays.fill(this.visible, null);
    }

    @Override
    public void remove() {
        // Remove o nome do NPC do gerenciador
        ...
        // Envia o pacote para jogadores antigos visíveis, se existirem
