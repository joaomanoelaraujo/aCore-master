...
import me.joaomanoel.d4rkk.dev.replay.ReplaySession;
import me.joaomanoel.d4rkk.dev.replay.ReplayingUtils;
...
public class Replayer {
    private final HashMap<String, INPC> npcs;
    private final HashMap<Integer, IEntity> entities;
    private final Map<Location, ItemData> blockChanges;
    ...
    private final ReplayingUtils utils;
    ...
    public void start() {
        ...
        if (data.getActions().containsKey(0)) {
            ...
            spawnData.ifPresent(value -> this.watcher.teleport(LocationData.toLocation(value.getLocation())));
        }
        ...
        this.run.runTaskTimer(Core.getInstance(), 1L, 1L);
    }
    ...
    public void stop() {
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(Core.getInstance(), profile::refresh);
