...
extends SubCommand {
    ...
    public boolean execute(final CommandSender cs, Command cmd, String label, String[] args) {
        ...
        if (!ReplaySaver.getReplays().isEmpty()) {
            int page = 1;
    ...
    private Comparator<String> dateComparator() {
        return (s1, s2) -> {
            if (this.getCreationDate(s1) != null && this.getCreationDate((String)s2) != null) {
                return Objects.requireNonNull(this.getCreationDate(s1)).compareTo(this.getCreationDate((String)s2));
            }
            return 0;
        };
    }
    ...
    public List<String> onTab(CommandSender cs, Command cmd, String label, String[] args) {
        return IntStream.range(1, new CommandPagination<String>(ReplaySaver.getReplays(), 9).getPages() + 1).boxed().map(String::valueOf).collect(Collectors.toList());
    }
}
