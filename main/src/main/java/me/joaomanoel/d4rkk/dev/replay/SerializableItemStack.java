...
implements Serializable {
    ...
    public static SerializableItemStack fromItemStack(ItemStack stack, boolean block) {
        ...
        serialized.entrySet().removeIf(e -> !((String)e.getKey()).equalsIgnoreCase("v") && !((String)e.getKey()).equalsIgnoreCase("type") && !((String)e.getKey()).equalsIgnoreCase("damage"));
        SerializableItemStack serializableItemStack = new SerializableItemStack(serialized);
        if (!block) {
            serializableItemStack.setHasEnchantment(!stack.getEnchantments().isEmpty());
