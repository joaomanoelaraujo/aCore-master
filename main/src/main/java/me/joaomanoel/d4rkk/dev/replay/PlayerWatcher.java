...
implements Serializable {
    ...
    private String getActivePose() {
        if (this.sneaking) {
            return VersionUtil.isCompatible(VersionUtil.VersionEnum.V1_14) ? "SNEAKING" : EnumWrappers.EntityPose.CROUCHING.toString();
        }
        if (this.swimming) {
            return EnumWrappers.EntityPose.SWIMMING.toString();
        }
        return EnumWrappers.EntityPose.STANDING.toString();
    }
