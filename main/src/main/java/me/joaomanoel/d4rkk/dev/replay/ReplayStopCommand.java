...
extends SubCommand {
    ...
    public List<String> onTab(CommandSender cs, Command cmd, String label, String[] args) {
        ...
        return ReplayManager.activeReplays.keySet().stream().filter(name -> StringUtil.startsWithIgnoreCase((String)name, args.length > 1 ? args[1] : null)).collect(Collectors.toList());
    }
}
