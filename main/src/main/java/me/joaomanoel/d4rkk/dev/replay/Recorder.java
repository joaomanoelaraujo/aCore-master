...
import me.joaomanoel.d4rkk.dev.replay.JsonData;
...
import me.joaomanoel.d4rkk.dev.replay.PacketRecorder;
import me.joaomanoel.d4rkk.dev.replay.PlayerInfo;
...
import me.joaomanoel.d4rkk.dev.replay.SkinInfo;
import me.joaomanoel.d4rkk.dev.replay.SpawnData;
import me.joaomanoel.d4rkk.dev.replay.WebsiteFetcher;
...
public class Recorder {
    ...
    public Recorder(Replay replay, List<Player> players, String gameName, String mode, String map, String server) {
        this.replay = replay;
        ...
        this.run = new BukkitRunnable(){
            ...
            public void run() {
                ...
                for (String name : tmpMap.keySet()) {
                    ArrayList list = new ArrayList(tmpMap.get(name));
                    for (Object obj : list) {
                        if (!(obj instanceof PacketData)) continue;
                        PacketData packetData = (PacketData) obj;

                        if (packetData instanceof BlockChangeData && !ConfigManager.RECORD_BLOCKS) continue;
                        if (packetData instanceof EntityItemData && ((EntityItemData) packetData).getAction() != 2 && !ConfigManager.RECORD_ITEMS) continue;
    ...
    public void stop(boolean save) {
        ...
        if (save) {
            for (String playera : this.players) {
                ...
                this.replay.setGameName(this.gameName);
                this.replay.setMode(this.mode);
                this.replay.setMap(this.map);
                this.replay.setServer(this.server);
                this.replay.setPlayers(this.playerCount);

                this.replay.setReplayInfo(new ReplayInfo(this.replay.getId(), this.players, System.currentTimeMillis(), this.currentTick, this.replay.getGameName(), this.replay.getMode(), this.replay.getMap(), this.replay.getServer(), this.replay.getPlayers()));
    ...
    public void createSpawnAction(final Player player, final Location loc, final boolean first) {
        ...
        Multimap<String, WrappedSignedProperty> map =
                WrappedGameProfile.fromPlayer(player).getProperties();
