...
import me.joaomanoel.d4rkk.dev.replay.IReplaySaver;
...
implements IReplaySaver {
    ...
    public void reformatAll() {
        ...
        if (DIR.exists()) {
            Arrays.stream(Objects.requireNonNull(DIR.listFiles())).filter(file -> file.isFile() && file.getName().endsWith(".replay")).map(File::getName).collect(Collectors.toList()).forEach(file -> this.reformat(file.replaceAll("\\.replay", "")));
        }
