...
@DataTableInfo(
    name = "aCoreThePit",
    create = "CREATE TABLE IF NOT EXISTS `aCoreThePit` (`name` VARCHAR(32), `kills` LONG, `deaths` LONG, `dailykits` TEXT, `assists` LONG, `experience` LONG, `level` LONG, `monthlykills` LONG, `monthlydeaths` LONG, `monthlyassists` LONG, `month` TEXT, `coins` DOUBLE, `laststreak` LONG, `killstreak` LONG, `cosmetics` TEXT, `selected` TEXT, PRIMARY KEY(`name`)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE utf8_bin;",
    select = "SELECT * FROM `aCoreThePit` WHERE LOWER(`name`) = ?",
    insert = "INSERT INTO `aCoreThePit` VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    update = "UPDATE `aCoreThePit` SET `kills` = ?, `deaths` = ?, `assists` = ?, `experience` = ?, `level` = ?, `monthlykills` = ?, `dailykits` = ?, `montlhydeaths` = ?, `monthlyassists` = ?, `month` = ?, `coins` = ?, `laststreak` = ?, `killstreak` = ?, `cosmetics` = ?, `selected` = ? WHERE LOWER(`name`) = ?"
)
public class KitPvPTable extends DataTable {
  ...
  public Map<String, DataContainer> getDefaultValues() {
    ...
    defaultValues.put("dailykits", new DataContainer("{}"));
