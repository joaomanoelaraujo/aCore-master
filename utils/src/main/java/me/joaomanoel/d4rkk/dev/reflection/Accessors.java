package me.joaomanoel.d4rkk.dev.reflection;
...
public class Accessors {
  ...
  /**
   * Seta o valor de um field através de um {@link FieldAccessor}
   *
   * @param field  O field para modificar.
  ...
  public static void setFieldValue(Field field, Object target, Object value) {
    new FieldAccessor<>(field).set(target, value);
  }
  ...
  public static Object getFieldValue(Field field, Object target) {
    return new FieldAccessor<>(field).get(target);
  }
   ...
   * @param methodName O nome do método, caso for null irá ignorar.
  ...
  public static <T> ConstructorAccessor<T> getConstructor(Class<T> clazz, int index, Class... parameters) {
    ...
    for (final Constructor<?> constructor : clazz.getDeclaredConstructors()) {
      if ((parameters == null || Arrays.equals(constructor.getParameterTypes(), parameters)) && index-- == 0) {
        return new ConstructorAccessor<>((Constructor<T>) constructor);
      }
    }
