package me.joaomanoel.d4rkk.dev.libraries;
...
public class MinecraftVersion {
  ...
  private void checkMinecraftVersion(JavaPlugin plugin) {
    MinecraftVersion currentVersion = MinecraftVersion.getCurrentVersion();
    MinecraftVersion version120 = new MinecraftVersion(1, 20, 6);

    // Verifica se a versão do servidor é 1.20.6
    if (currentVersion.equals(version120)) {
      // Utiliza o NMS do módulo nms-1.20.6

    } else {
      plugin.getLogger().warning("Versão não compatível com o módulo nms-1.20.6. Versão atual: " + currentVersion);
    }
  }
